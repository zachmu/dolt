...
func TestTableDiff(t *testing.T) {
	...
	root2, err := root.PutTable(context.Background(), ddb.ValueReadWriter(), "tbl1", tbl1)
	assert.NoError(t, err)
	...
	root3, err := root.PutTable(context.Background(), ddb.ValueReadWriter(), "tbl1", tbl1Updated)
	assert.NoError(t, err)
	...
	root4, err := root3.PutTable(context.Background(), ddb.ValueReadWriter(), "tbl2", tbl1)
	assert.NoError(t, err)
