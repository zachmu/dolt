...
func setupMergeTest() (types.ValueReadWriter, *doltdb.Commit, *doltdb.Commit, types.Map, types.Map) {
	...
	mRoot, err = mRoot.PutTable(context.Background(), ddb.ValueReadWriter(), tableName, tbl)

	if err != nil {
		panic(err)
	}

	updatedRoot, err := mRoot.PutTable(context.Background(), ddb.ValueReadWriter(), tableName, updatedTbl)

	if err != nil {
		panic(err)
	}

	mergeRoot, err := mRoot.PutTable(context.Background(), ddb.ValueReadWriter(), tableName, mergeTbl)
