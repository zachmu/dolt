	...
	for _, loc := range testLocations {
		t.Run(loc.String(), func(t *testing.T) {
			...
			if tableVal, isTable := loc.(TableDataLocation); isTable {
				...
				assert.NoError(t, err)
				root, err = root.PutTable(context.Background(), ddb.ValueReadWriter(), tableVal.Name, tbl)
				assert.NoError(t, err)
...
func TestCreateRdWr(t *testing.T) {
	...
	for _, test := range tests {
		...
		if nomsWr, ok := wr.(noms.NomsMapWriteCloser); ok {
			...
			root, err = root.PutTable(context.Background(), ddb.ValueReadWriter(), tableLoc.Name, tbl)
			assert.NoError(t, err)
		}
